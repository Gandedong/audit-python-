...
很多计算机语言中，有很多相同名称的函数，如Print函数，无论是C语言，还是B语言，他们的函数名称都是一样的，但可能在输入参数会有些不一样，所以有一定的计算机语言基础下，去学另一种计算机语言不会太难。我如果将函数比喻为招式，很多不同门派的武学，他们的基础招式都是一样的，直拳就是直拳，冲拳就是冲拳，但是内里会有一些微细的不同，这个不同点就是参数，也就是变化。
...
dir()
运行结果如下：
['In', '_i28','_i29','_i3','_i30','_i31','_i32','_i33','_i4','_i5','_i6','_i' '_i8','_i9', '_ih','_ii','_iii','_oh','exit','get_ipython','pvA','quit',
...
所以在《黑客帝国》之中最后一部就叫做《矩阵革命》其实不正确，应该叫做《数组革命》。因为矩阵是一个数学概念（线性代数里的），数组是个计算机上的概念。
于是在接下来的发展中，我估计对数组的计算就越出色，就越有机会成为人工智能的计算机语言，而Python之所以被选作人工智能的开发语言，因为他的Numpy库可以做到多维数组的计算。
相比较Python的内置函数，NumPy库能更快，更紧凑的计算数组，使用起来也很方便。除了在人工智能领域，在经济学、金融学、统计学上，数组的计算也是非常重要。
我举一个不是很出名的例子进行说明一下，如投入产出计算，这个是诺贝尔经济学奖获奖者提出的一种经济分析计算方法。
...
X为变量，u为总体平均值，N为总体例数。当数据分布比较分散（即数据在期望值附近波动较大）时，各个数据与期望值的差的平方和较大，方差就较大，反之，当数据分布比较集中时，各个数据与期望值的差的平方和较小。方差就越少。
